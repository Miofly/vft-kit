---
name: fe-auto-test
description: "验证前端页面、控制台、Canvas/WebGL、交互与截图，运行 Lighthouse 和缓存/hydration 审计；浏览器优先 ego-lite，批量回归用脚本。纯静态分析不用。"
---
...
```bash
bash "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/check-deps.sh"
```
...
```bash
bash "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/check-server.sh" [项目目录]
```
...
```bash
node "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/lighthouse-audit.mjs" <url> \
  --device=mobile --resources=/tmp/res.json [--q='?debug=true']
# 再把资源清单喂给拆解脚本：
python3 "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/resource-report.py" /tmp/res.json
```
...
> ```bash
> node "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/seo-audit.mjs" <url> [40] [--all] [--q='?debug=true']
> ```
...
8. 把上一步的落盘路径喂给 bundled 脚本，一步出「资源拆解 + 缓存压缩实测」：
   ```bash
   python3 "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/resource-report.py" <analyze_resources落盘路径>
   ```
...
```bash
bash "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/cleanup-screenshots.sh"
```
...
- 若 dev server 是**你为这次验证启动的**，关掉它：
  ```bash
  bash "${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/close-server.sh" <端口>
  ```
...
>
> **⚠️ 后台运行必须用脚本绝对路径**：`run_in_background` 的 bash 继承的是**上一条命令 cd 到的目录**（常是项目根，不是 scripts 目录），`node route-audit.mjs ...` 会 `MODULE_NOT_FOUND`。后台跑一律写全路径，如 `node /Users/.../skills/dev-workflow/fe-auto-test/scripts/route-audit.mjs ...`（或先在同一条命令里 `S=<绝对路径>` 再 `node "$S"`）。
>
...
**脚本目录**：`${VFT_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-${CODEX_PLUGIN_ROOT:-}}}/skills/dev-workflow/fe-auto-test/scripts/`
