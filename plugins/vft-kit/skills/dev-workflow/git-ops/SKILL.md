...
| 需求 | 按需读取 | 交付 |
|---|---|---|
| 生成提交信息、提交、Commit 规范检查 | [commits.md](references/commits.md) | 基于实际 diff 的 message、检查结果；授权提交后返回 SHA |
| 冲突解决、分支管理、同步与恢复 | [branches-conflicts.md](references/branches-conflicts.md) | 保留双方意图的合并、定向验证、最终 Git 状态 |
| Code Review、PR 描述、创建 PR/MR | [review-pr.md](references/review-pr.md) | 有证据的 findings、可直接使用的正文、创建后的远端回读 |
...
- 疑似真实密钥停止提交/推送，只报告脱敏的文件位置。文件名筛查不等于内容安全扫描；优先复用仓库 secret scanner，示例配置也要检查值。
...
按本次范围简报：改动/审查对象、验证结果、当前分支与提交 SHA、剩余修改；PR 另报 URL、base/head、远端 head SHA。未执行的检查明确标注，不编造通过状态。
