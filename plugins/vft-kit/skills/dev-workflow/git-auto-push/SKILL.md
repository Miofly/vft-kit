---
name: git-auto-push
description: "仅在用户明确要求绕过 Git hooks、--no-verify 或不跑 lint 提交推送时使用，执行 commit + push。"
---
...
2. 改动集中在一个目录 → `<type>(<目录名>): <动作> xxx`
   ...
   - 例：只改 `skills/dev-workflow/git-auto-push/SKILL.md` → `chore: 更新 git-auto-push skill`
