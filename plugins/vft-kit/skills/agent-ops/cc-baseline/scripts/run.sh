...
CG_FILTER_SCRIPT="${CG_FILTER_SCRIPT:-$SCRIPT_DIR/install-codegraph-prompt-filter.sh}"
...
if ! bash "$CG_FILTER_SCRIPT"; then
  printf '  ○ codegraph prompt-hook 过滤安装异常；继续执行基线检查\n' >&2
fi
