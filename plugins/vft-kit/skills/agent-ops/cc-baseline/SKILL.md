...
`run.sh` 先保证 RTK 可用（缺失时 Homebrew 安装）并把 RTK 的 `PreToolUse` Bash hook 幂等补进 `~/.claude/settings.json`（`rtk hook claude`，装了才会自动压缩 Bash 输出），再幂等安装或更新并启用 Caveman 插件，把 `~/.config/caveman/config.json.defaultMode` 固定为 `full`，再把 codegraph 的 `UserPromptSubmit` hook 包上过滤脚本（后台任务通知和非代码问题不再注入约 15 KB 符号，省 token），最后调用只读的 `check.sh`。退出码为 0 表示必需项齐全，为 1 表示存在必需项缺失；可选项不影响退出码。
...
- RTK（含其 PreToolUse hook）、Ponytail、Caveman 默认 full 和 codegraph prompt-hook 过滤缺失已获长期授权，可直接补齐；其他项目先回报。
...
`scripts/test-*.sh` 共 8 个，全部用 `mktemp` 造隔离的假 `HOME` 与假 `PATH` 运行，**不碰真实环境、不装任何东西**。改动 `check.sh` / `run.sh` 后必跑：
...
覆盖：`run.sh` 的安装顺序 / 参数转发 / 退出码传播、RTK 二进制与 hook 的幂等安装、Caveman 默认 full、codegraph 与 code-review-graph 与 Emil skills 的基线判定、RTK hook 缺失判为必需项、codegraph prompt-hook 过滤（筛选逻辑与幂等改写 settings）。

> 测试靠环境变量注入 mock（`RTK_SCRIPT` / `CAVEMAN_SCRIPT` / `CHECK_SCRIPT`），所以 `run.sh` 里这些路径变量（含 `CG_FILTER_SCRIPT`）**必须保持 `${VAR:-默认值}` 形式**；写成硬编码会让 mock 注入失效、测试报出误导性的「exit code not propagated」。
