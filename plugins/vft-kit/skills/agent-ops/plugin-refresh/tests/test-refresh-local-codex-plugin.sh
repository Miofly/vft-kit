...
mkdir -p "$PLUGIN_ROOT/.codex-plugin" "$PLUGIN_ROOT/skills/a" "$PLUGIN_ROOT/skills/b" "$PLUGIN_ROOT/skills/group/c" "$FAKE_BIN" "$CODEX_HOME"
...
printf '%s\n' '---' 'name: c' '---' '# C' > "$PLUGIN_ROOT/skills/group/c/SKILL.md"
...
[ -f "$CODEX_HOME/plugins/cache/vft-kit/vft-kit/0.0.1-test/skills/group/c/SKILL.md" ] || { printf 'FAIL: nested cache skill c absent\n' >&2; exit 1; }
