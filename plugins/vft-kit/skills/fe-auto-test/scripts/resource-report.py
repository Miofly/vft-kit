...
def normalize(res):
    """补齐两条路径的字段差异：filename 缺就从 url 推；type 统一小写。

    lighthouse-audit.mjs 落盘的 type 是大写（Script/Stylesheet/Font），MCP analyze_resources
    是小写。下游所有按 type 的筛选（stylesheet 明细、图片明细、缓存实测）都按小写写，
    不统一就在脚本路径下整块变空——这是缓存实测"探测 0 个"的真凶。
    """
    for r in res:
        r["type"] = (r.get("type") or "other").lower()
...
def main():
    ...
    if do_probe:
        ...
        for r in targets:
            ...
            enc = p.get("encoding")
            flag = ""
            long_cache = cc and ("immutable" in cc or "max-age=315" in cc)
            if not cc:
                flag += " ❗无Cache-Control(每次都协商)"
            elif not long_cache:
                flag += " ⚠缓存偏短"
            # 长缓存 + 文件名无 content-hash = 定时炸弹：发版后 URL 不变，用户被 CDN/浏览器钉在旧版本。
            # content-hash 段是 base64（几乎必含大写字母，vite hash 常带 -/_，如 vft-CNK-Rye8）或长数字串；
            # 纯小写 kebab 语义名（tools-header-search）才算无 hash。别用「结尾 8+ 位字母数字」判——会被 kebab 段误伤。
            else:
                stem = r.get("filename", "").rsplit(".", 1)[0]
                if not (re.search(r"[A-Z]", stem) or re.search(r"\d{5,}", stem)):
                    flag += " ⚠长缓存却无hash(发版不更新,危险)"
