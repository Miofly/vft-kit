...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/check-deps.sh
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/check-server.sh [项目目录]
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/lighthouse-audit.mjs <url> \
  --device=mobile --resources=/tmp/res.json [--q='?debug=true']
# 再把资源清单喂给拆解脚本：
python3 ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/resource-report.py /tmp/res.json
```
...
8. 把上一步的落盘路径喂给 bundled 脚本，一步出「资源拆解 + 缓存压缩实测」：
   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/resource-report.py <analyze_resources落盘路径>
   ```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/cleanup-screenshots.sh
```
...
- 若 dev server 是**你为这次验证启动的**，关掉它：
  ```bash
  bash ${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/close-server.sh <端口>
  ```
...
**脚本目录**：`${CLAUDE_PLUGIN_ROOT}/skills/fe-auto-test/scripts/`
