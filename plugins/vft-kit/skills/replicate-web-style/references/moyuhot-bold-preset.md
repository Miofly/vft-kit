...
- Bold toast: use an ink surface with paper text, a 2–2.5px ink border, about 10px radius, slight sticker rotation, and one 4px hard shadow colored by status (green success, pink error, yellow warning, blue info). Animate only transform and opacity, and use a service-supported per-call class instead of an unscoped global message override.
- Local skeleton: keep the final component's border, radius, paper surface, grid footprint, and hard shadow so loading does not shift layout. Use warm low-contrast content bars with one continuous shimmer; reserve the route-level loading card for initial navigation and use skeletons for slower page sections. Scope generic skeleton helpers under the page root and stop the shimmer under reduced motion.
