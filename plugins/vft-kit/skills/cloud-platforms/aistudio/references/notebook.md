...
1. Use the free CPU runtime for every GPU-free step: pin source, download and install the exact target stack, run CPU-capable checks, validate/preprocess datasets, prepare selected model files, and write checksums plus a handoff manifest. CUDA wheels can be installed and imported on CPU; only a real CUDA device probe belongs on GPU.
2. Publish reusable dependency archives, wheelhouses, model files, and preprocessed data to a same-account private Dataset or model repository. Do not populate `/home/aistudio` with an unpacked venv or model weights solely for a later switch; Home snapshots with many files or multiple GB can make the switch slower or fail.
3. After the GPU runtime is ready, restore only files used by the selected graph, verify the CPU manifest and hashes, then run the exact offline import probe and real CUDA operation. Disable package/model downloads; if anything is missing, stop the paid runtime and return to CPU preparation instead of installing it on GPU.
4. Benchmark candidate mirrors with a small/ranged transfer before the CPU download, then keep one mirror for that preparation run. Record timestamps for CPU download/install/publish and GPU restore, service readiness, prompt submission, sampling, decode, and output verification.
...
Keep `/home/aistudio` small: scripts, configs, manifests, short logs, and markers only. Put reusable wheels/archives and model weights in a same-account private Dataset or model repository; restore them into the mounted or ephemeral data path for the current run. Do not unpack a venv, pip cache, or model snapshot into Home before an environment switch. Before switching, inspect both file count and size, remove failed partial downloads, and remotely read back every persisted artifact; retain the manifest and logs so the next run can resume at the failed phase.
