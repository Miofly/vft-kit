...
`hooks/hooks.json` 只被 Claude Code 加载。规则与检查逻辑都在 `scripts/rules/`，Codex 侧通过 CLI 调用同一份实现：`check.mjs <file...>` 检查、`inject.mjs <file>` 打印适用规则。新增规则只写 `rules/*.md` 与 `scripts/rules/checks/`，不要为两个平台各写一份。
