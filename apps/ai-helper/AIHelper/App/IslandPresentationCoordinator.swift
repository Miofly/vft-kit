...
final class IslandPresentationCoordinator {
    ...
    private var bannerObserver: BannerCompletionObserver?
    private var bannerNotchUsage: BannerNotchUsageController?
    ...
    func applySurfaceMode(
        ...
        // 离开 banner 模式时停掉横幅观察者 + 刘海用量
        if mode != .banner {
            bannerObserver?.stop()
            bannerNotchUsage?.unmount()
        }
        ...
        case .banner:
            presentBannerSurface()
        }
    }

    /// banner 模式:拆掉刘海/宠物窗口,桌面只留菜单栏 + 事件横幅
    private func presentBannerSurface() {
        ...
        if bannerObserver == nil {
            bannerObserver = BannerCompletionObserver(sessionMonitor: sessionMonitor)
        }
        bannerObserver?.start()

        // 有物理刘海时,挂上「划到刘海看用量」(控制器内部自检刘海,无刘海则空操作)
        if bannerNotchUsage == nil {
            bannerNotchUsage = BannerNotchUsageController()
        }
        bannerNotchUsage?.mount()
    }
