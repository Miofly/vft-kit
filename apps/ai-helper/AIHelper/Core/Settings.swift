...
enum IslandSurfaceMode: String, CaseIterable, Identifiable {
    ...
    var subtitle: String {
        ...
        case .banner:
            return "桌面无常驻刘海/宠物，仅在需要介入或对话结束时弹出毛玻璃横幅"
        }
    }
}
...
final class AppSettingsStore: ObservableObject {
    ...
    private enum Keys {
        ...
        static let bannerDurationSeconds = "bannerDurationSeconds"
        static let bannerNotifyCompleted = "bannerNotifyCompleted"
        static let bannerNotifyEnded = "bannerNotifyEnded"
        static let bannerNotifyAttention = "bannerNotifyAttention"
        static let bannerNotifyError = "bannerNotifyError"
        static let bannerNotifyCompacted = "bannerNotifyCompacted"
    ...
    /// 「自绘横幅」展示模式下横幅停留秒数
    @Published var bannerDurationSeconds: Double {
        didSet {
            let clamped = min(max(bannerDurationSeconds, 2), 12)
            if bannerDurationSeconds != clamped {
                bannerDurationSeconds = clamped
                return
            }
            guard !isBootstrapping else { return }
            defaults.set(bannerDurationSeconds, forKey: Keys.bannerDurationSeconds)
        }
    }

    // 自绘横幅:按事件类型的弹出开关
    @Published var bannerNotifyCompleted: Bool {
        didSet { guard !isBootstrapping else { return }; defaults.set(bannerNotifyCompleted, forKey: Keys.bannerNotifyCompleted) }
    }
    @Published var bannerNotifyEnded: Bool {
        didSet { guard !isBootstrapping else { return }; defaults.set(bannerNotifyEnded, forKey: Keys.bannerNotifyEnded) }
    }
    @Published var bannerNotifyAttention: Bool {
        didSet { guard !isBootstrapping else { return }; defaults.set(bannerNotifyAttention, forKey: Keys.bannerNotifyAttention) }
    }
    @Published var bannerNotifyError: Bool {
        didSet { guard !isBootstrapping else { return }; defaults.set(bannerNotifyError, forKey: Keys.bannerNotifyError) }
    }
    @Published var bannerNotifyCompacted: Bool {
        didSet { guard !isBootstrapping else { return }; defaults.set(bannerNotifyCompacted, forKey: Keys.bannerNotifyCompacted) }
    }
    ...
    init(
        ...
        _bannerDurationSeconds = Published(initialValue: Self.doubleValue(
            from: defaults,
            key: Keys.bannerDurationSeconds,
            exists: persistedKeys.contains(Keys.bannerDurationSeconds),
            default: 5
        ))
        _bannerNotifyCompleted = Published(initialValue: Self.boolValue(
            from: defaults, key: Keys.bannerNotifyCompleted,
            exists: persistedKeys.contains(Keys.bannerNotifyCompleted), default: true))
        _bannerNotifyEnded = Published(initialValue: Self.boolValue(
            from: defaults, key: Keys.bannerNotifyEnded,
            exists: persistedKeys.contains(Keys.bannerNotifyEnded), default: true))
        _bannerNotifyAttention = Published(initialValue: Self.boolValue(
            from: defaults, key: Keys.bannerNotifyAttention,
            exists: persistedKeys.contains(Keys.bannerNotifyAttention), default: true))
        _bannerNotifyError = Published(initialValue: Self.boolValue(
            from: defaults, key: Keys.bannerNotifyError,
            exists: persistedKeys.contains(Keys.bannerNotifyError), default: true))
        _bannerNotifyCompacted = Published(initialValue: Self.boolValue(
            from: defaults, key: Keys.bannerNotifyCompacted,
            exists: persistedKeys.contains(Keys.bannerNotifyCompacted), default: false))
...
enum AppSettings {
    ...
    static var bannerDurationSeconds: Double {
        get { shared.bannerDurationSeconds }
        set { shared.bannerDurationSeconds = newValue }
    }

    static var bannerNotifyCompleted: Bool { get { shared.bannerNotifyCompleted } set { shared.bannerNotifyCompleted = newValue } }
    static var bannerNotifyEnded: Bool { get { shared.bannerNotifyEnded } set { shared.bannerNotifyEnded = newValue } }
    static var bannerNotifyAttention: Bool { get { shared.bannerNotifyAttention } set { shared.bannerNotifyAttention = newValue } }
    static var bannerNotifyError: Bool { get { shared.bannerNotifyError } set { shared.bannerNotifyError = newValue } }
    static var bannerNotifyCompacted: Bool { get { shared.bannerNotifyCompacted } set { shared.bannerNotifyCompacted = newValue } }
