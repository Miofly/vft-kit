...
#if APP_STORE
        "Mac App Store 版本不会自动写入 ~/.claude、~/.codex 等目录。选择安装时，ai-helper 会请求你授权用户主目录后再写入配置。"
#else
        "ai-helper 通过 Hooks 监听会话事件、显示通知与审批。可以一键安装默认配置，或选择仅启用部分事件。"
#endif
    }
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 22) {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text(appLocalized: "你可以把 ai-helper 放在屏幕顶部，也可以让宠物默认贴近当前激活窗口右下角显示。之后都能在设置里随时切换。")
                ...
                Toggle(isOn: $analyticsOptIn) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text(appLocalized: "帮助提升 ai-helper 体验")
