...
final class SettingsPanelViewModel: ObservableObject {
    ...
    func exportLogs() {
        ...
        panel.nameFieldStringValue = "AIHelper-Diagnostics-\(Self.archiveTimestamp()).zip"
...
private struct SoundSettingsContent: View {
    ...
    private let notificationEvents: [NotificationEvent] = [.attentionRequired, .taskCompleted]
        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            if settings.soundThemeMode == .builtIn {
                SoundEventSection(title: "阶段音效") {
                    ForEach(Array(notificationEvents.enumerated()), id: \.element.id) { index, event in
                        ...
                        if index < notificationEvents.count - 1 {
                            SettingsLineDivider()
                        }
                    }
                }
...
private struct SettingsPanelContentView: View {
    ...
    var body: some View {
        ...
        .alert(
            AppLocalization.string("帮助提升 ai-helper 体验？"),
    ...
    private var displayContent: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsSectionCard(title: "面板") {
                ...
                if settings.surfaceMode == .notch {
                ...
                } else if settings.surfaceMode == .floatingPet {
                    SettingsLineDivider()
                    ...
                    SettingsToggleLine(
                        title: "需要介入时弹横幅",
                        subtitle: "等待审批、回答问题等需要你接手的时刻",
                        isOn: $settings.bannerNotifyAttention
                    )
                    SettingsLineDivider()
                    SettingsSliderLine(
                        title: "横幅显示时长",
                        subtitle: "横幅停留的秒数，到时自动淡出；横幅可拖动到任意位置并会被记住。提示音在「声音」里配。",
                        value: $settings.bannerDurationSeconds,
                        range: 2...12,
                        step: 1,
                        format: { "\(Int($0.rounded())) 秒" }
                    )
                }
                SettingsLineDivider()
    ...
    private func replayNotchDetachmentHint() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            NotificationCenter.default.post(name: .aiHelperPresentNotchDetachmentHint, object: nil)
        }
    }
    ...
    private var integrationContent: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsSectionCard(title: "Hook 调试日志") {
                SettingsToggleLine(
                    title: "记录 Hook 调试日志",
                    subtitle: "关闭后 bridge 不再追加 ~/.ai-helper-debug 下的 hook 调试记录，并在下次 hook 触发时清理既有日志。",
                ...
                SettingsSliderLine(
                    title: "最大日志占用",
                    subtitle: "当 ~/.ai-helper-debug 超过该大小时，会优先删除最旧的 hook 调试日志。",
                    value: Binding(
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            SettingsSectionCard(title: "更新") {
                ...
                if updateManager.canInstallPendingUpdateNow {
                    ...
                    SettingsActionLine(
                        title: "立即重启安装",
                        subtitle: "不等待空闲，立即退出 ai-helper 并完成已下载的更新"
                    ) {
            ...
            SettingsSectionCard(title: "链接") {
                SettingsActionLine(title: "GitHub", subtitle: "打开 Issues 页面反馈问题") {
                    if let url = URL(string: "https://github.com/Miofly/vft-kit/issues") {
                        NSWorkspace.shared.open(url)
                    }
                } accessory: {
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text(appLocalized: "如果不再需要远端集成，可在这里直接卸载 bridge；这会删除远端 `~/.ai-helper` 并撤回 Island 托管的 hooks。")
                }
...
struct IslandSurfaceModeSelector: View {
    ...
    var subtitle: String? = "选择 ai-helper 的主显示方式。你随时可以在设置里切换，并立即看到新的渲染效果。"
...
private struct IslandSurfaceModePreviewScene: View {
    ...
    private func bannerPreview(in size: CGSize) -> some View {
        let bannerW = min(max(size.width * 0.62, 120), 176)
        return ZStack(alignment: .topTrailing) {
            VStack {
                HStack {
                    Text(appLocalized: "自绘横幅")
                        .font(.system(size: 10, weight: .semibold))
                        .foregroundColor(.white.opacity(0.46))
                    Spacer()
                }
            ...
            HStack(spacing: 7) {
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .fill(
                        LinearGradient(
                            colors: [Color(red: 0.42, green: 0.86, blue: 0.62),
                                     Color(red: 0.20, green: 0.62, blue: 0.44)],
                ...
                VStack(alignment: .leading, spacing: 3) {
                    RoundedRectangle(cornerRadius: 1.5, style: .continuous)
                        ...
                        .frame(width: bannerW * 0.42, height: 3)
                    RoundedRectangle(cornerRadius: 1.5, style: .continuous)
                        .fill(Color.white.opacity(0.40))
                        .frame(width: bannerW * 0.66, height: 3)
                }
                Spacer(minLength: 0)
            }
            .padding(.horizontal, 9)
            .padding(.vertical, 8)
            .frame(width: bannerW, alignment: .leading)
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
