...
enum SessionCompletionPreviewBuilder {
    ...
    static func sanitized(_ text: String?) -> String? {
        ...
        // 移除常见 markdown 格式标记
        // 标题：移除行首或空格后的 # 标记（处理 "## title" 和 " ## title"）
        result = result.replacingOccurrences(of: #"(^|\s)#{1,6}\s+"#, with: "$1", options: .regularExpression)
        // 粗体/斜体/代码
        result = result.replacingOccurrences(of: #"\*\*(.+?)\*\*"#, with: "$1", options: .regularExpression)  // 粗体 **text**
        result = result.replacingOccurrences(of: #"__(.+?)__"#, with: "$1", options: .regularExpression)  // 粗体 __text__
        result = result.replacingOccurrences(of: #"\*([^*]+?)\*"#, with: "$1", options: .regularExpression)  // 斜体 *text*
        result = result.replacingOccurrences(of: #"_([^_]+?)_"#, with: "$1", options: .regularExpression)  // 斜体 _text_
        result = result.replacingOccurrences(of: #"`(.+?)`"#, with: "$1", options: .regularExpression)  // 行内代码 `code`
        // 链接
        result = result.replacingOccurrences(of: #"\[(.+?)\]\(.+?\)"#, with: "$1", options: .regularExpression)  // 链接 [text](url)
