...
actor DiagnosticsExporter {
    ...
    func exportArchive(to destinationURL: URL) async throws -> DiagnosticsExportResult {
        ...
        let tempRoot = fileManager.temporaryDirectory
            .appendingPathComponent("AIHelper-Diagnostics-\(UUID().uuidString)", isDirectory: true)
        let exportRoot = tempRoot.appendingPathComponent("AIHelper-Diagnostics-\(timestamp)", isDirectory: true)
        var warnings: [String] = []
    ...
    private func writeRedactedTailCopy(from sourceURL: URL, to destinationURL: URL) throws {
        ...
        var output = result.wasTruncated
            ? "[ai-helper diagnostics: file truncated to last \(Self.maxDebugFileBytes) bytes]\n"
            : ""
    ...
    private func writeTailCopy(from sourceURL: URL, to destinationURL: URL) throws {
        ...
        if result.wasTruncated {
            output.append(Data("[ai-helper diagnostics: file truncated to last \(Self.maxDebugFileBytes) bytes]\n".utf8))
        }
    ...
    private func copyRecentCrashReports(toRelativeDirectory relativePath: String, under rootURL: URL) throws {
        ...
        .filter { url in
            ...
            return name.hasPrefix("aihelper") || name.hasPrefix("aihelper")
        }
        .sorted { lhs, rhs in
            let leftDate = (try? lhs.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate) ?? .distantPast
            let rightDate = (try? rhs.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate) ?? .distantPast
    ...
    private func writeUnifiedLogs(to destinationURL: URL) async throws {
        let predicate = "subsystem == \"com.wfly.aihelper\""
        try await writeCommandOutput(
    ...
    private func preferredCodexHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/codex-hooks", isDirectory: true)
    }

    private func preferredClaudeHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/claude-hooks", isDirectory: true)
    }

    private func preferredCodeBuddyHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/codebuddy-hooks", isDirectory: true)
    }

    private func preferredCodeBuddyCLIHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/codebuddy-cli-hooks", isDirectory: true)
    }

    private func preferredQoderHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/qoder-hooks", isDirectory: true)
    }

    private func preferredQoderCLIHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/qoder-cli-hooks", isDirectory: true)
    }

    private func preferredHermesHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/hermes-hooks", isDirectory: true)
    }

    private func preferredGeminiHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ai-helper-debug/gemini-hooks", isDirectory: true)
    }
