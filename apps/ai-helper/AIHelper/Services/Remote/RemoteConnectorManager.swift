...
final class RemoteConnectorManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.wfly.aihelper", category: "Remote")
    ...
    private func writeRemoteBridgeLauncher(endpoint: RemoteEndpoint, password: String?) async throws {
        let launcherPath = "\(endpoint.remoteInstallRoot)/bin/ai-helper-bridge"
        try await RemoteSSHCommandRunner.writeRemoteFileViaSSH(
            target: endpoint.sshTarget,
            port: endpoint.sshPort,
    ...
    private func bootstrapRemoteAgent(endpointID: UUID, password: String?, probe: RemoteHostProbe) async throws {
        ...
        let stagedBridgePath = "\(endpoint.remoteInstallRoot)/bin/AIHelperBridge.tmp"
        ...
        let bridgeBinaryPath = "\(endpoint.remoteInstallRoot)/bin/AIHelperBridge"
        let launcherPath = "\(endpoint.remoteInstallRoot)/bin/ai-helper-bridge"
        ...
        if bridgeAlreadyInstalled {
            ...
            do {
                ...
                let manualHint = """
                SCP 传输失败，请在远程主机上手动安装 AIHelperBridge：
                
                  mkdir -p \(endpoint.remoteInstallRoot)/bin
                  curl -L -o /tmp/bridge.zip https://github.com/Miofly/vft-kit/releases/latest/download/AIHelperBridge-linux-musl-\(arch).zip
                  unzip -o /tmp/bridge.zip -d \(endpoint.remoteInstallRoot)/bin/
                  chmod 755 \(endpoint.remoteInstallRoot)/bin/AIHelperBridge
        ...
        for profile in remoteHookProfiles {
            let remoteCommand = HookInstaller.managedBridgeCommand(
                ...
                launcherPath: "\(endpoint.remoteInstallRoot)/bin/ai-helper-bridge",
    ...
    nonisolated static func remoteLinuxBridgeBinaryAssetName(normalizedArchitecture: String) -> String {
        "AIHelperBridge-linux-musl-\(normalizedArchitecture)"
    }
    ...
    nonisolated static func remoteLinuxBridgeLegacyBinaryAssetName(normalizedArchitecture: String) -> String {
        "AIHelperBridge-linux-\(normalizedArchitecture)"
    }
    ...
    nonisolated static func remoteBootstrapPrepareCommand(
        ...
        sleep 1
        rm -f \(shellQuote(controlSocketPath)) \(shellQuote(hookSocketPath)) \(shellQuote("\(installRoot)/bin/AIHelperBridge.tmp"))
        """
    }
    ...
    nonisolated static func remoteEnsureAgentRunningCommand(
        ...
        if [ ! -x \(shellQuote("\(installRoot)/bin/ai-helper-bridge")) ] || [ ! -x \(shellQuote("\(installRoot)/bin/AIHelperBridge")) ]; then
          echo "ai-helper remote bridge is not installed at \(installRoot)/bin" >&2
          exit 127
        fi
        pkill -f \(shellQuote(servicePattern)) >/dev/null 2>&1 || true
        rm -f \(shellQuote(controlSocketPath)) \(shellQuote(hookSocketPath))
        nohup \(shellQuote("\(installRoot)/bin/ai-helper-bridge")) --mode remote-agent-service --hook-socket \(shellQuote(hookSocketPath)) --control-socket \(shellQuote(controlSocketPath)) > \(shellQuote("\(installRoot)/logs/remote-agent.log")) 2>&1 &
        sleep 1
        if [ -S \(shellQuote(controlSocketPath)) ] && pgrep -f \(shellQuote(servicePattern)) >/dev/null 2>&1; then
          exit 0
        fi
        echo "ai-helper remote bridge failed to start" >&2
    ...
    nonisolated static func remoteBootstrapInstallCommand(
        ...
        """
        mv -f \(shellQuote(stagedBridgePath)) \(shellQuote("\(installRoot)/bin/AIHelperBridge"))
        chmod 755 \(shellQuote("\(installRoot)/bin/AIHelperBridge")) \(shellQuote("\(installRoot)/bin/ai-helper-bridge"))
        """
    }
    ...
    nonisolated static func remoteBridgeLauncherScript() -> String {
        ...
        if [ -x "$COMPAT_LIB/ld-linux-x86-64.so.2" ] && ldd "$SCRIPT_DIR/AIHelperBridge" 2>&1 | grep -q 'libc\\.so'; then
          exec "$COMPAT_LIB/ld-linux-x86-64.so.2" --library-path "$COMPAT_LIB" "$SCRIPT_DIR/AIHelperBridge" "$@"
        fi
        exec "$SCRIPT_DIR/AIHelperBridge" "$@"
        """
    }
    ...
    nonisolated static func remoteManagedBridgeArguments(
        ...
        [
            "\(installRoot)/bin/ai-helper-bridge",
            "--source",
...
private struct RemoteEndpointCredentialStore {
    private let service = "com.wfly.aihelper.remote-host-password"
...
private enum RemoteConnectorError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .localBridgeBinaryMissing:
            return "本地 AIHelperBridge 二进制不存在，无法安装到远程主机"
        case .missingClaudeHookProfile:
...
private final class RemoteAttachConnector {
    nonisolated private static let logger = Logger(subsystem: "com.wfly.aihelper", category: "Remote")
    ...
    func start() async throws {
        ...
        let process = try RemoteSSHCommandRunner.makeSSHProcess(
            ...
            port: endpoint.sshPort,
            password: password,
            remoteCommand: "\(shellQuote("\(endpoint.remoteInstallRoot)/bin/ai-helper-bridge")) --mode remote-agent-attach --control-socket \(shellQuote(endpoint.remoteControlSocketPath))",
            acceptNewHostKey: true
        )
...
private enum RemoteSSHCommandRunner {
    private static let logger = Logger(subsystem: "com.wfly.aihelper", category: "RemoteSSH")
    ...
    static func writeRemoteFile(target: String, port: Int, remotePath: String, contents: Data, password: String?) async throws {
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ai-helper-remote-\(UUID().uuidString)")
    ...
    private static func sshEnvironment(password: String?) throws -> [String: String] {
        ...
        let askpassURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ai-helper-askpass-\(UUID().uuidString)")
        let script = """
        #!/bin/sh
        ...
        environment["DISPLAY"] = environment["DISPLAY"] ?? "ai-helper:0"
        return environment
    }
...
private final class RemoteBridgeAssetResolver {
    ...
    private func downloadLinuxBridge(for architecture: String) async throws -> URL {
        ...
        let cacheDirectory = fileManager.homeDirectoryForCurrentUser
            .appendingPathComponent(".ai-helper", isDirectory: true)
        ...
        for candidate in assetCandidates {
            let cachedArchiveURL = cacheDirectory.appendingPathComponent(candidate.archive)
            let releaseURLString = "https://github.com/Miofly/vft-kit/releases/download/ai-helper-v\(version)/\(candidate.archive)"
