...
struct CodexUsageSnapshot: Equatable, Codable, Sendable {
    ...
    nonisolated var hasUsageData: Bool {
        !windows.isEmpty || tokenUsage != nil
    }
}
...
enum CodexUsageLoader {
    ...
    private nonisolated static func snapshot(from line: String, filePath: String, fallbackTimestamp: Date) -> CodexUsageSnapshot? {
        ...
        let windows = ["primary", "secondary"].compactMap { key in
            rateLimits.flatMap { usageWindow(for: key, in: $0) }
        }
        let tokenUsage = tokenUsage(from: payload["info"])
        guard !windows.isEmpty || tokenUsage != nil else {
            return nil
        }
        ...
        return CodexUsageSnapshot(
            ...
            planType: rateLimits.flatMap { string(from: $0["plan_type"]) },
            limitID: rateLimits.flatMap { string(from: $0["limit_id"]) },
