...
final class QoderCLIVersionDetectionTests: XCTestCase {
    ...
    func testQoderHookRefreshPreservesUnrelatedSettings() throws {
        ...
        let existing = """
        {
          ...
          "hooks": {
            "PreToolUse": [
              ...
              {
                "matcher": "*",
                "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder"}]
              }
            ],
            "PostToolUseFailure": [
              {
        ...
        let ideData = HookInstaller.updatedConfigurationData(
            ...
            customCommand: "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder",
            installing: true
        )
        let data = HookInstaller.updatedConfigurationData(
            existingData: ideData,
            profile: qoderCLIProfile,
            customCommand: "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder-cli --client-name Qoder CLI --client-origin cli --client-originator Qoder",
            installing: true
        )
        ...
        XCTAssertEqual(
            commands.filter { $0.contains("/.ai-helper/bin/ai-helper-bridge") }.count,
            2
        )
    ...
    func testCodeBuddyCLIHookRefreshPreservesCodeBuddyIDEHooks() throws {
        ...
        let existing = """
        {
          ...
          "hooks": {
            "PreToolUse": [
              ...
              {
                "matcher": "*",
                "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind codebuddy --client-name CodeBuddy --client-originator CodeBuddy"}]
              }
            ]
          }
        }
        ...
        let ideData = HookInstaller.updatedConfigurationData(
            ...
            customCommand: "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind codebuddy --client-name CodeBuddy --client-originator CodeBuddy",
            installing: true
        )
        let data = HookInstaller.updatedConfigurationData(
            existingData: ideData,
            profile: codeBuddyCLIProfile,
            customCommand: "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind codebuddy-cli --client-name 'CodeBuddy CLI' --client-origin cli --client-originator CodeBuddy",
            installing: true
        )
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: data) as? [String: Any])
        XCTAssertEqual(json["theme"] as? String, "dark")
