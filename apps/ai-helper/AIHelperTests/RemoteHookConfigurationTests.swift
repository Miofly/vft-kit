...
final class RemoteHookConfigurationTests: XCTestCase {
    func testRemoteBootstrapPrepareCommandStopsRunningAgentBeforeReplacingBridge() {
        let command = RemoteConnectorManager.remoteBootstrapPrepareCommand(
            installRoot: "/root/.ai-helper",
            controlSocketPath: "/root/.ai-helper/run/agent-control.sock",
            hookSocketPath: "/root/.ai-helper/run/agent-hook.sock",
            homeDirectory: "/root",
        ...
        XCTAssertTrue(command.contains("AIHelperBridge"))
        XCTAssertTrue(command.contains("rm -f "))
        XCTAssertTrue(command.contains("AIHelperBridge.tmp"))
    }

    func testRemoteBootstrapPrepareCommandResolvesClaudeDirectoryAgainstHome() {
        let command = RemoteConnectorManager.remoteBootstrapPrepareCommand(
            installRoot: "/home/dev/.ai-helper",
            controlSocketPath: "/home/dev/.ai-helper/run/agent-control.sock",
            hookSocketPath: "/home/dev/.ai-helper/run/agent-hook.sock",
    ...
    func testRemoteBootstrapInstallCommandPromotesStagedBridgeAtomically() {
        let command = RemoteConnectorManager.remoteBootstrapInstallCommand(
            installRoot: "/root/.ai-helper",
            stagedBridgePath: "/root/.ai-helper/bin/AIHelperBridge.tmp"
        )

        XCTAssertTrue(command.contains("mv -f '/root/.ai-helper/bin/AIHelperBridge.tmp' '/root/.ai-helper/bin/AIHelperBridge'"))
        XCTAssertTrue(command.contains("chmod 755 '/root/.ai-helper/bin/AIHelperBridge' '/root/.ai-helper/bin/ai-helper-bridge'"))
    }
    ...
    func testRemoteBridgeLauncherOnlyUsesCompatLoaderForDynamicGlibcBridge() {
        ...
        XCTAssertTrue(script.contains("ldd \"$SCRIPT_DIR/AIHelperBridge\" 2>&1 | grep -q 'libc\\.so'"))
        XCTAssertTrue(script.contains("exec \"$SCRIPT_DIR/AIHelperBridge\" \"$@\""))
    }
    ...
    func testRemoteEnsureAgentRunningCommandReplacesStaleSocketStateBeforeRestart() {
        ...
        XCTAssertTrue(command.contains("mkdir -p '/root/.ai-helper/run' '/root/.ai-helper/logs'"))
        XCTAssertTrue(command.contains("if [ -S '/root/.ai-helper/run/agent-control.sock' ] && pgrep -f '/root/.ai-helper/bin/[P]ingIslandBridge --mode remote-agent-service' >/dev/null 2>&1; then"))
        XCTAssertTrue(command.contains("ai-helper remote bridge is not installed at /root/.ai-helper/bin"))
        XCTAssertTrue(command.contains("pkill -f '/root/.ai-helper/bin/[P]ingIslandBridge --mode remote-agent-service' >/dev/null 2>&1 || true"))
        XCTAssertTrue(command.contains("rm -f '/root/.ai-helper/run/agent-control.sock' '/root/.ai-helper/run/agent-hook.sock'"))
        XCTAssertTrue(command.contains("nohup '/root/.ai-helper/bin/ai-helper-bridge' --mode remote-agent-service --hook-socket '/root/.ai-helper/run/agent-hook.sock' --control-socket '/root/.ai-helper/run/agent-control.sock' > '/root/.ai-helper/logs/remote-agent.log' 2>&1 &"))
        XCTAssertTrue(command.contains("ai-helper remote bridge failed to start"))
        XCTAssertTrue(command.contains("tail -n 40 '/root/.ai-helper/logs/remote-agent.log'"))
    }
    ...
    func testRemoteBootstrapUninstallCommandStopsBridgeAndRemovesInstallRoot() {
        ...
        XCTAssertTrue(command.contains("pkill -f '/root/.ai-helper/bin/[P]ingIslandBridge --mode remote-agent-service'"))
        XCTAssertTrue(command.contains("pkill -f '/root/.ai-helper/bin/[P]ingIslandBridge --mode remote-agent-attach'"))
        XCTAssertTrue(command.contains("rm -f '/root/.ai-helper/run/agent-control.sock' '/root/.ai-helper/run/agent-hook.sock'"))
        XCTAssertTrue(command.contains("rm -rf '/root/.ai-helper'"))
    }
    ...
    func testRemoteManagedHookConfigDirectoryPathsResolveUnderRemoteHome() {
        ...
        XCTAssertTrue(directories.contains("/root/.openclaw/hooks/ai-helper-openclaw"))
    ...
    func testManagedConfigurationDataInstallsClaudeHooksWithoutRemovingExistingEntries() throws {
        ...
        let command = HookInstaller.managedBridgeCommand(
            ...
            launcherPath: "/remote/bin/ai-helper-bridge",
    ...
    func testManagedConfigurationDataRemovesLocalOnlyCommandsForRemoteInstall() throws {
            ...
            "UserPromptSubmit": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "/Users/ai-helper/.claude/hooks/peon-ping/scripts/hook-handle-use.sh"
                  }
                ]
              },
              {
        ...
        let data = HookInstaller.updatedConfigurationData(
            existingData: existingJSON,
            profile: profile,
            customCommand: "ISLAND_SOCKET_PATH='/root/.ai-helper/run/agent-hook.sock' '/root/.ai-helper/bin/ai-helper-bridge' --source claude",
            installing: true,
        ...
        XCTAssertFalse(commands.contains { $0.contains("/Users/ai-helper/.claude/hooks/peon-ping") })
        XCTAssertTrue(commands.contains("/usr/bin/echo keep"))
        XCTAssertTrue(commands.contains { $0.contains("/root/.ai-helper/bin/ai-helper-bridge") })
    }
            ...
            "UserPromptSubmit": [
              ...
              },
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "ISLAND_SOCKET_PATH='/remote/run/agent-hook.sock' '/home/test/.ai-helper/bin/ai-helper-bridge' --source claude"
                  }
                ]
              }
            ]
    ...
    func testOpenClawInternalHookConfigurationDataEnablesManagedEntry() throws {
        ...
        let entry = try XCTUnwrap(entries["ai-helper-openclaw"] as? [String: Any])
    ...
    func testOpenClawRemoteBridgeArgumentsAndEnvironmentUseRemoteInstallRootAndSocket() throws {
        ...
        XCTAssertEqual(arguments.prefix(3), [
            "/root/.ai-helper/bin/ai-helper-bridge",
            "--source",
            "claude",
        ])
        ...
        XCTAssertEqual(
            environment,
            ["ISLAND_SOCKET_PATH": "/root/.ai-helper/run/agent-hook.sock"]
        )
    }
