    ...
    func testPublishedGitHubFeedURLIsAccepted() {
        XCTAssertTrue(
            UpdateManager.isValidFeedURL(
                "https://github.com/Miofly/vft-kit/releases/latest/download/appcast.xml"
            )
        )
    }
        ...
        let error = NSError(
            ...
            userInfo: [
                ...
                NSUnderlyingErrorKey: NSError(
                    ...
                    code: Int(SUError.downloadError.rawValue),
                    userInfo: [
                        NSLocalizedDescriptionKey: "A network error occurred while downloading https://github.com/wfly/Island/releases/latest/download/appcast.xml. not found (404)"
                    ]
                )
            ]
        )
