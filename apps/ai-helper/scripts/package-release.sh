...
DMG_BACKGROUND_SOURCE="${PING_ISLAND_DMG_BACKGROUND_SOURCE:-$PROJECT_DIR/docs/images/ai-helper-dmg-installer-background.png}"
DMG_LOGO_SOURCE="${PING_ISLAND_DMG_LOGO_SOURCE:-$PROJECT_DIR/docs/images/ai-helper-icon-transparent.svg}"

APP_BUNDLE_NAME="ai-helper.app"
APP_PRODUCT_NAME="AIHelper"
...
NOTARY_KEYCHAIN_PROFILE="${PING_ISLAND_NOTARY_KEYCHAIN_PROFILE:-${NOTARY_KEYCHAIN_PROFILE:-AIHelper}}"
...
resolve_exported_app_icon() {
    ...
    echo "$PROJECT_DIR/AIHelper/Assets.xcassets/AppIcon.appiconset/icon_1024x1024.png"
}
...
find_sparkle_bin_dir() {
    ...
    local candidates=(
        ...
        "$HOME/Library/Developer/Xcode/DerivedData/AIHelper-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
...
validate_release_progression() {
    ...
    previous_release_tsv=$(RELEASES_JSON="$releases_json" python3 - "$VERSION" "$RELEASE_TAG_PREFIX" <<'PY'
import json
import os
import sys

current_tag = f"{sys.argv[2]}{sys.argv[1]}"
...
for release in releases:
    ...
    if (
        ...
        or tag_name == current_tag
        or not tag_name.startswith(sys.argv[2])
    ):
        continue
    ...
    zip_url = next(
        (
            ...
            if asset.get("name", "").startswith("AIHelper-")
...
PY
    ...
    unzip -p "$tmp_dir/previous.zip" "ai-helper.app/Contents/Info.plist" > "$previous_plist"
...
echo "=== Packaging Signed ai-helper ==="
echo ""
...
rm -f "$ZIP_PATH" "$DMG_PATH" "$DMG_PATH.sha256" "$NOTARY_ZIP_PATH"
rm -rf "$STAGING_DIR"
...
create_styled_dmg "$APP_PATH" "$DMG_PATH" "ai-helper" "$STAGING_DIR" "$PROJECT_DIR"
...
log_section "Creating SHA-256 Checksum"
