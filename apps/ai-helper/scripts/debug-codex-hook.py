...
When AIHelperBridge is available, it also forwards the hook payload so the
normal ai-helper session flow keeps working during debugging.

It is intentionally side-effect light:
- writes newline-delimited JSON records to disk
- forwards stdout/stderr from AIHelperBridge when forwarding is enabled
...
DEFAULT_LOG_DIR = Path.home() / ".ai-helper-debug" / "codex-hooks"
DEFAULT_BRIDGE_PATH = Path.home() / ".ai-helper" / "bin" / "ai-helper-bridge"
