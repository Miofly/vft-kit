<h1 align="center">
  <img src="docs/images/ai-helper-icon.svg" width="64" height="64" alt="ai-helper 应用图标" valign="middle">&nbsp;
  ai-helper
</h1>
<p align="center">
  <b>macOS 菜单栏里的灵动岛风格 AI 编码会话监视器</b><br>
  <a href="https://github.com/Miofly/vft-kit">仓库</a> •
...
<p align="center">
  <img src="docs/images/notch-panel.png" width="960" alt="ai-helper 预览图">
</p>
<p align="center">
...
<p align="center">
  <sub>源码：<a href="https://github.com/Miofly/vft-kit/tree/main/apps/ai-helper">Miofly/vft-kit/apps/ai-helper</a></sub>
</p>
...
<p align="center">
  <img src="docs/images/demos/ai-helper-ask-tool-demo.gif" width="800" alt="ai-helper 宠物离岛交互演示">
</p>

在刘海屏 Mac 上，当 Agent 需要你处理时，ai-helper 会从刘海处展开，带上会话上下文和可操作按钮。

<p align="center">
  <img src="docs/images/demos/ai-helper-question-demo.gif" width="800" alt="ai-helper 刘海屏交互演示">
</p>
...
安装插件后，对 Claude Code 或 Codex 说“安装 ai-helper”。`install-ai-helper` skill 会下载正式 DMG，并校验 SHA-256、Bundle ID、Developer ID 签名和 Gatekeeper；不需要下载源码或安装 Xcode。

### 下载发行版

1. 打开 [vft-kit Releases](https://github.com/Miofly/vft-kit/releases)，选择 `ai-helper-v*` 标签
2. 下载 `AIHelper-<版本>.dmg`
3. 将 `ai-helper.app` 拖到 Applications
4. 启动应用，并打开你希望 ai-helper 监控的客户端
...
默认会使用仓库里的 `docs/images/ai-helper-dmg-installer-background.png` 作为 DMG 安装背景；如果你想在本地预览别的背景图，可以临时设置 `PING_ISLAND_DMG_BACKGROUND_SOURCE`。

维护者可运行 `./scripts/create-release.sh` 产出带 `Developer ID` 签名、公证和 SHA-256 校验文件的安装包，并发布为 `ai-helper-v<版本>` 标签。

## ai-helper 是什么？

ai-helper 是一个 macOS 菜单栏应用。当你的编码 Agent 需要你处理审批、输入或查看结果时，它会展开成一个紧凑的会话界面。它能接 Claude 风格 hooks、Codex hooks、Gemini CLI hooks、Hermes Agent plugin hooks、Pi Agent extension hooks、Qwen Code hooks、Kimi CLI hooks、OpenClaw internal hooks + session transcripts、Codex app-server、OpenCode 插件，以及兼容 IDE 的集成层，所以你不用一直盯着终端标签页，也能看到会话状态。

如果你了解过 [Vibe Island](https://vibeisland.app/)，可以把 ai-helper 理解成同一产品方向下的独立开源替代方案：它同样是一个原生 macOS 刘海区 / 菜单栏界面，用来监控和控制 AI 编码会话。
...
ai-helper 关注的，是那些真正会打断编码节奏的时刻，并把它们用原生 macOS 刘海界面接住。
...
- **SSH 终端支持** - 可以通过 SSH 自动引导远程 AIHelperBridge，把远程 Claude 兼容 hooks 重写到桥接入口，并把远程终端里的事件统一回流到你本机的 Island 界面。
...
- **额度一眼可见** - 菜单栏同时汇总 Claude 与 Codex 用量；套餐登录可查看 Codex 5h / 7d 的额度与重置倒计时，API Key 登录则显示最近本地会话的 Token 总量。
...
| --- | --- | --- | --- |
| Claude Code | 通过 `AIHelperBridge` 接入 Claude 兼容 hooks | Terminal.app、iTerm2、Ghostty、tmux、IDE 内终端 | 工具审批、AskUserQuestion 回复、压缩提醒、完成弹窗、自动批准 |
...
如果 `AIHelperUITests-Runner` 在 macOS 上一直停在 suspended，优先在 Xcode 里用有效本地签名身份跑 UI 测试，并结合 `amfid` / `AppleSystemPolicy` 日志判断是不是代码签名或系统策略问题。

## 设置面板

ai-helper 当前提供 4 个设置分类：
...
ai-helper 在 `设置 -> Sound` 里提供三种声音模式：
...
ai-helper 也会自动发现放在 `~/.openpeon/packs` 和 `~/.claude/hooks/peon-ping/packs` 下面的主题包。
...
主题包里的音频文件支持 `.wav`、`.mp3`、`.ogg`。如果当前主题包没有提供某个事件对应的分类，ai-helper 会回退到该事件当前选中的 macOS 系统音。
...
- Claude 家族工具主要通过托管 hook 文件和内嵌的 `AIHelperBridge` 启动器接入。
...
- 远程 SSH 主机可以自动引导 `AIHelperBridge`，重写远程 Claude 兼容 hooks 指向桥接入口，并把远程事件回流到本机 ai-helper。
...
- 安装你希望 ai-helper 监控的 CLI 或桌面客户端

## 致谢

ai-helper 延续了 [claude-island](https://github.com/farouqaldori/claude-island) 这类刘海式 Agent 监视器的思路，并把它扩展到了多客户端 hooks、Codex app-server 同步和 IDE 路由能力上。
