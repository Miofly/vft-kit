...
func installerMergesClaudeHooksWithoutDroppingExistingValues() throws {
    ...
    #expect(sessionStartCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude") })
...
@Test
func installerCreatesLauncherUnderAIHelperSupportDirectory() throws {
    let root = URL(fileURLWithPath: NSTemporaryDirectory()).appending(path: UUID().uuidString, directoryHint: .isDirectory)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    ...
    let launcherURL = root.appending(path: ".ai-helper/bin/ai-helper-bridge")
    ...
    #expect(launcher.contains("AIHelperBridge"))
...
func installerDeduplicatesManagedHooksButKeepsUnrelatedHooks() throws {
    ...
    let claudeExisting = """
    {
      "hooks": {
        "PreToolUse": [
          {
            "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude"}],
            "matcher": "*"
          },
          {
            "hooks": [{"type": "command", "command": "/usr/bin/true"}],
    ...
    let codexExisting = """
    {
      "hooks": {
        "UserPromptSubmit": [
          {
            "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source codex"}],
            "matcher": "*"
          },
          {
    ...
    let qoderExisting = """
    {
      "hooks": {
        "PostToolUseFailure": [
          {
            "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder --client-name Qoder --client-originator Qoder"}],
            "matcher": "*"
          },
          {
    ...
    let qoderWorkExisting = """
    {
      "hooks": {
        "PostToolUseFailure": [
          {
            "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoderwork --client-name QoderWork --client-originator QoderWork"}],
            "matcher": "*"
          },
          {
    ...
    let workBuddyExisting = """
    {
      "hooks": {
        "PostToolUse": [
          {
            "hooks": [{"type": "command", "command": "/Users/test/.ai-helper/bin/ai-helper-bridge --source claude --client-kind workbuddy --client-name WorkBuddy --client-originator WorkBuddy"}],
            "matcher": "*"
          },
          {
    ...
    #expect(preToolUseCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude") })
    #expect(preToolUseCommands.filter { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude") }.count == 1)
    ...
    #expect(codexCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source codex") })
    #expect(codexCommands.filter { $0.contains("/.ai-helper/bin/ai-helper-bridge --source codex") }.count == 1)
    ...
    #expect(qoderPostToolUseFailureCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder --client-name Qoder --client-originator Qoder") })
    ...
    let qoderIDECommand = "/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder --client-name Qoder --client-originator Qoder"
    let qoderCLICommand = "/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoder-cli --client-name 'Qoder CLI' --client-origin cli --client-originator Qoder"
    ...
    #expect(qoderWorkCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoderwork --client-name QoderWork --client-originator QoderWork") })
    #expect(qoderWorkCommands.filter { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoderwork --client-name QoderWork --client-originator QoderWork") }.count == 1)
    ...
    let qoderWorkManagedPreToolUse = try #require(
        qoderWorkPreToolUse.first {
            (((($0["hooks"] as? [[String: Any]])?.first)?["command"] as? String) ?? "")
                .contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind qoderwork --client-name QoderWork --client-originator QoderWork")
        }
    )
    ...
    #expect(workBuddyCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind workbuddy --client-name WorkBuddy --client-originator WorkBuddy") })
    #expect(workBuddyCommands.filter { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind workbuddy --client-name WorkBuddy --client-originator WorkBuddy") }.count == 1)
...
func installerAddsCodeBuddyWorkBuddyAndCursorHooks() throws {
    ...
    #expect(codeBuddyCommands.contains {
        $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind codebuddy --client-name CodeBuddy --client-originator CodeBuddy")
    })
    let codeBuddyCLICommand = "/.ai-helper/bin/ai-helper-bridge --source claude --client-kind codebuddy-cli --client-name 'CodeBuddy CLI' --client-origin cli --client-originator CodeBuddy"
    ...
    #expect(workBuddyCommands.contains {
        $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind workbuddy --client-name WorkBuddy --client-originator WorkBuddy")
    })
    #expect(workBuddyHooks["SessionEnd"] != nil)
    #expect(workBuddyHooks["PreCompact"] != nil)
    #expect(workBuddyHooks["PermissionRequest"] == nil)
    ...
    #expect(cursorCommands.contains {
        $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude --client-kind cursor --client-name Cursor --client-originator Cursor")
    })
...
func installerAcceptsJSONCSettingsFiles() throws {
    ...
    #expect(commands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source claude") })
}
...
func installerAddsCopilotHooksUsingGitHubFormat() throws {
    ...
    let existing = """
    {
      ...
      "hooks": {
        "sessionStart": [
          ...
          {
            "type": "command",
            "bash": "/Users/test/.ai-helper/bin/ai-helper-bridge --source copilot --event sessionStart"
          }
        ]
      }
    }
    ...
    #expect(sessionStartCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source copilot --event sessionStart") })
    ...
    #expect(preToolUseCommands.contains { $0.contains("/.ai-helper/bin/ai-helper-bridge --source copilot --event preToolUse") })
}
