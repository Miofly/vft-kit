...
struct IDEExtensionInstaller {
    ...
    static let extensionIdentifier = "ai-helper.session-focus"
    private static let extensionIconFilename = "icon.png"
    private static let extensionReadmeFilename = "README.md"
    private static let projectHomepage = "https://github.com/Miofly/vft-kit"
    private static let projectRepository = "https://github.com/Miofly/vft-kit.git"
    private static let projectIssues = "https://github.com/Miofly/vft-kit/issues"
    ...
    private func extensionJS(sessionFocusStrategy: SessionFocusStrategy?) -> String {
            ...
            sessionFocusLogic = """
                async function focusChatSession(sessionId) {
                    ...
                    try {
                        ...
                        console.warn('[ai-helper] Failed to focus chat session', sessionId, error);
                        return false;
                    }
                }
            """
        ...
        function logInfo(message, details) {
            if (details === undefined) {
                console.info(`[ai-helper] ${message}`);
                return;
            }

            console.info(`[ai-helper] ${message}`, JSON.stringify(details));
        }

        function logWarn(message, details) {
            if (details === undefined) {
                console.warn(`[ai-helper] ${message}`);
                return;
            }

            console.warn(`[ai-helper] ${message}`, JSON.stringify(details));
        }
        ...
        async function handleSetupURI(uri) {
            ...
            await vscode.window.showInformationMessage('ai-helper is ready.');
        }
    ...
    private func extensionReadme(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        let capabilityLine = sessionFocusStrategy == .qoderChatHistory
            ...
            : "It reopens the matching terminal tab from ai-helper's session context."

        return """
        # ai-helper

        ai-helper installs this VS Code-compatible extension so the app can jump back into the right IDE window for your active coding session.

        \(capabilityLine)

        Manage installs, reinstalls, and authorization from ai-helper's **Settings -> Integration** panel.
    ...
    private static func applicationVersion() -> String {
        let projectURL = repositoryRootURL().appending(path: "AIHelper.xcodeproj/project.pbxproj")
        guard
    ...
    private func repositoryAppIconURL() -> URL {
        Self.repositoryRootURL()
            .appending(path: "AIHelper/Assets.xcassets/AppIcon.appiconset/icon_128x128.png")
    }

    private func extensionDescription(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        sessionFocusStrategy == nil
            ? "Lets ai-helper focus the matching terminal tab"
            : "Lets ai-helper focus the matching chat session or terminal tab"
    }

    private func vsixManifest(sessionFocusStrategy: SessionFocusStrategy?) -> String {
        let description = sessionFocusStrategy == nil
            ? "Lets ai-helper focus the matching terminal tab in VS Code compatible IDEs."
            : "Lets ai-helper focus the matching chat session or terminal tab in VS Code compatible IDEs."

        return """
        <?xml version="1.0" encoding="utf-8"?>
        <PackageManifest Version="2.0.0" xmlns="http://schemas.microsoft.com/developer/vsx-schema/2011">
          <Metadata>
            <Identity Language="en-US" Id="session-focus" Version="\(Self.extensionVersion)" Publisher="ai-helper"/>
            <DisplayName>ai-helper</DisplayName>
