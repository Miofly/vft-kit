...
struct IslandBridgeMain {
    ...
    static func main() async {
        ...
        do {
            ...
            FileHandle.standardError.write(Data("AIHelperBridge error: \(error.localizedDescription)\n".utf8))
            Foundation.exit(1)
        }
    }
...
private enum BridgeDebugLogger {
    ...
    private static func debugLocation(for target: String, environment: [String: String]) -> DebugLogLocation {
        ...
        let rootDirectory = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".ai-helper-debug", isDirectory: true)
...
private enum SocketClient {
    private static let healthCheckRequest = #"{"type":"ai-helper-health-check"}"#
...
private final class RemoteAgentService: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.wfly.aihelper.remote-agent", qos: .userInitiated)
